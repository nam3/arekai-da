...
describe 'dominator_supreme_with_nightmare', ->
  ...
  context "with input = \"#{baseInput} --item1=\"#{item1}\" --item2=\"#{item2}\" --item3=\"#{item3}\" --from=#{from} --size=#{size} --payment-method=#{paymentMethod} --attempts=#{attempts} --interval=#{interval} --max-checkout-delay=#{maxCheckoutDelay} --users=#{users}\"", ->

    it 'gets image alt of item1, item2 and item3, from, paymentMethod, attempts, interval, maxCheckoutDelay and users', ->

      match = regex.exec "#{baseInput} --item1=\"#{item1}\" --item2=\"#{item2}\" --item3=\"#{item3}\" --from=#{from} --size=#{size} --payment-method=#{paymentMethod} --attempts=#{attempts} --interval=#{interval} --max-checkout-delay=#{maxCheckoutDelay} --users=#{users}"
      expect match[1]
        .to.be.equal item1
      expect match[2]
        .to.be.equal item2
