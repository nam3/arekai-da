...
#
# Commands:
#   arekai-da: dominator nike "SHOE_URL" --size=e.g:27, 27.5 --from=1986-12-02T00:00:00 --interval=2000(milliseconds) --times=30 --concurrency=1
#
# Author:
#   JumpeiArashi
...
module.exports = (robot) ->
  ...
  robot.respond /dominator\s+nike\s+(https?:\/\/[\w/:%#$&?()~.=+_-]+)(?:\s*--size=([\d.]+)|)(?:\s*--from=([T\d:-]+)|)(?:\s*--interval=([\d]+)|)(?:\s*--times=([\d]+)|)(?:\s*--concurrency=([\d]+)|)$/, (res) ->
    ...
    Promise.all(utils.generatePromises(Number(res.match[6] or 1), ->
      register = new Register(res.match[3], (res.match[4] or 2000), (res.match[5] or 60), 'buy nike shoe', userId, res.match[1], res.match[2], utils.isDryrun())
