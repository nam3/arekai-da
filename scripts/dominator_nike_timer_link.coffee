# Description
#   Purchase Nike Shoe when timer link with dominator
#
# Configuration:
#   MONGODB_USER
#   MONGODB_PASSWORD
...
#
# Commands:
#   arekai-da: dominator nike-timer-link "SHOE_URL" --from=1986-12-02T00:00:00 --size=e.g:27, 27.5 --attempts=10 --interval=2000(milliseconds) --concurrency=1 --users=user001,user002
#
# Author:
#   JumpeiArashi
...
module.exports = (robot) ->
  ...
  robot.respond /dominator\s+nike-timer-link\s+(https?:\/\/[\w/:%#$&?()~.=+_-]+)(?:\s*--from=([T\d:-]+)|)(?:\s*--size=([\d.:]+)|)(?:\s*--attempts=([\d]+)|)(?:\s*--interval=([\d]+)|)(?:\s*--concurrency=([\d]+)|)(?:\s*--users=([\w,]+)|)$/, (res) ->
